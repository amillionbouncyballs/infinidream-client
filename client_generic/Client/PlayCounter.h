...
class CPlayCounter : public Base::CSingleton<CPlayCounter>
{
...
#ifdef WIN32
            pdata->PlayCountFile =
                _fsopen(generation_path.string().c_str(), "r+b",
                        _SH_DENYWR); // deny write
#else
            pdata->PlayCountFile =
                fopen(generation_path.string().c_str(), "r+b"); // deny write
#endif
            if (pdata->PlayCountFile == NULL) // file not found
            {
#ifdef WIN32
                pdata->PlayCountFile = _fsopen(generation_path.string().c_str(),
                                               "w+b", _SH_DENYWR);
#else
                pdata->PlayCountFile =
                    fopen(generation_path.string().c_str(), "w+b");
#endif
                if (pdata->PlayCountFile == NULL) // unable to open for writing
                {
                    pdata->PlayCountFile =
                        fopen(generation_path.string().c_str(), "rb");
                    if (pdata->PlayCountFile ==
                        NULL) // unable to open for reading
                    {
                        m_ReadOnly = true;
                        g_Log->Error("Running without play counts: %s",
                                     generation_path.string().c_str());
                                            ...
                                            max_sheep, pdata->PlayCountFile))
                    {
                        m_ReadOnly = true;
                        g_Log->Error(
                            "Cannot initialize play counts, fwrite failed");
  ...
  public:
    CPlayCounter()
        : m_ReadOnly(false), m_DeadEndCutSurvivors(0), m_MedianCutSurvivors(0),
          m_PlayCountTotal(0)
    {
        m_PlayCountDecayY =
            g_Settings()->Get("settings.player.PlayCountDecayY", 2000);
        m_PlayCountDecayZ =
            g_Settings()->Get("settings.player.PlayCountDecayZ", 60);
    ...
    void clearDeadEndSurvivorsStats() { m_DeadEndCutSurvivors = 0; }

    void IncMedianCutSurvivors() { ++m_MedianCutSurvivors; }
    ...
    void IncPlayCount(uint32_t generation, uint32_t id)
    {
        ...
        {
            m_PlayCountTotal = 0;
            for (PlayCountMap::iterator jj = m_PlayCounts.begin();
    ...
    bool ReadOnlyPlayCounts() { return m_ReadOnly; }
};
