...
*/
CImage::CImage()
    : m_Format(eImage_None), m_Width(0), m_Height(0), m_nMipMaps(0),
      m_bRef(false)
{
}
...
void CImage::Create(const uint32_t _w, const uint32_t _h,
                    ...
                    const bool _bRef)
{
    g_Log->Debug( "CImage Create( %d, %d, 0x%x )", _w, _h, _format );
...
uint8_t* CImage::GetData(const uint32_t _mipLevel) const
{
    ...
    return (_mipLevel < m_nMipMaps)
               ? ab->GetBufferPtr() + getMipMappedSize(0, _mipLevel)
               : NULL;
}
...
{ int w = getWidth (firstMipMapLevel); int h = getHeight(firstMipMapLevel); int
d = getDepth (firstMipMapLevel); int size = 0; while (nMipMapLevels){ size += w
* h * d; w >>= 1; h >>= 1; d >>= 1; if (w + h + d == 0) break; if (w == 0) w =
1; if (h == 0) h = 1; if (d == 0) d = 1;
...
bool CImage::Load(const std::string& _fileName, const bool _calcMipmaps)
{
    ...
    if (ext == "")
    {
        g_Log->Warning("CImage::Load() No extension found for %s",
                       _fileName.c_str());
        return (false);
    }
    ...
    if (foundExt == false)
    {
        g_Log->Warning("CImage::Load() Unknown extension for %s",
                       _fileName.c_str());
        return (false);
    }
...
bool CImage::Save(const std::string& _fileName)
{
    ...
    if (ext == "")
    {
        g_Log->Warning("CImage::Save() No extension found for %s",
                       _fileName.c_str());
        return (false);
    }
    ...
    g_Log->Info("CImage::Save( %s ): Complete!", (const char*)_fileName.c_str(),
                m_Width, m_Height, m_nMipMaps);
    return (true);
}
...
bool CImage::SaveDDS(const std::string& _fileName)
{
    ...
    if ((fmt < eImage_I8 || fmt > eImage_RGBA8) && fmt != eImage_RGB565 &&
    ...
    DDSHeader header = {
        ...
        DDSD_CAPS | DDSD_PIXELFORMAT | DDSD_WIDTH | DDSD_HEIGHT |
            static_cast<unsigned int>(m_nMipMaps > 1 ? DDSD_MIPMAPCOUNT : 0),
        m_Height,
        m_Width,
        0,
        0,
        ...
        m_Format.isCompressed()
            ? DDPF_FOURCC
            : static_cast<unsigned int>((nChannels < 3) ? 0x00020000
                                                        : DDPF_RGB) |
                  static_cast<unsigned int>((nChannels & 1) ? 0
                                                            : DDPF_ALPHAPIXELS),
        ...
        (nChannels >= 3)   ? 0xff000000
        : (nChannels == 2) ? 0xFF00
                           : 0,
        DDSCAPS_TEXTURE |
            static_cast<unsigned int>(
                m_nMipMaps > 1 ? DDSCAPS_MIPMAP | DDSCAPS_COMPLEX : 0),
        0,
        {0, 0, 0},
    };
                       ...
                       uint32_t height)
{
    ...
    for (y = 0; y < height; y += 2)
    {
        for (x = 0; x < width; x += 2)
        {
            r = (((src[0] >> 8) & 0xF8) + ((src[xOff] >> 8) & 0xF8) +
                 ((src[diff] >> 8) & 0xF8) + ((src[diff + xOff] >> 8) & 0xF8));
            g = (((src[0] >> 3) & 0xFC) + ((src[xOff] >> 3) & 0xFC) +
                 ((src[diff] >> 3) & 0xFC) + ((src[diff + xOff] >> 3) & 0xFC));
    ...
    case eImage_Nearest:
        for (y = 0; y < _newHeight; y++)
        {
            ...
            for (x = 0; x < _newWidth; x++)
            {
                ...
                {
                    *dest++ =
                        pData[(sampleY * m_Width + sampleX) * nChannels + k];
                }
            }
        }
        break;
    ...
    case eImage_Bicubic:
        ...
        for (y = 0; y < _newHeight; y++)
        {
            ...
            for (x = 0; x < _newWidth; x++)
            {
                ...
                src = pData +
                      ((sampleY - 1) * m_Width + (sampleX - 1)) * nChannels;

                for (k = 0; k < nChannels; k++)
                {
                    b = icerp(src[m_Width * nChannels],
                              src[(m_Width + 1) * nChannels],
                    ...
                    c = icerp(src[2 * m_Width * nChannels],
